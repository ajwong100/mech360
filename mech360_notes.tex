...
\end{align*}
Note, we are using a right-handed frame to achieve these signs.
\\
...
\textbf{Example 8 Steps:} For the given section, write the steps to find the bending stress and maximum bending stress.
\begin{enumerate}
  \item Know/compute $\tensor{I}$. If $x$-$y$ are principal axes, $I_{xy}\neq0$, which is an easy problem.
  To find $I_{ij}$ where $i$ and $j$ represent $x,y$, or $z$, we find the centroid of each simple section, then apply the parallel axis theorem.
  \item Draw Mohr's Circle to find principal moments of inertia and principal axes. In principal centroidal axis $\tensor{I_p} = \m{I_u}{0}{0}{I_v}$.
  Note, $I_{uv}=0$.
  \item Superposition: $\sigma(u,v) = -\frac{M_v u}{I_v} + \frac{M_u v}{I_u}$ on an element at $(u,v)$.
  Since we can observe the direction of the moment and find a point $A: (u_A, v_A)$ that is farthest away from the neutral axis and causes each term in each direction to be of the same sign.
  Then, the max bending stress will be given by $|\sigma_{\text{max}}| = \sigma(u_A, v_A)$. If unsure, you can try multiple points $(u_i, v_i)$ and take max$\{\sigma(u_i,v_i)\}$.
\end{enumerate}
...
\section{Shear Stresses in Beams and Thin-Walled Members}
Consider the case when $M$ not constant, $V = \frac{dM}{dx} \neq 0$. This is not pure bending since
there is a varying bending moment.
\begin{itemize}
  \item Shear stresses: $\tau = \frac{V}{A}$ is actually wrong
  \item Shear Stress Formula: 
\end{itemize}
\textbf{What induces shear stresses?}\\
\dots \\
Most importantly: Always ask what is the shear plane? Visualize shear action, and
calculate shear area.
\subsection{Shear Flow}
\begin{itemize}
  \item $q$ is shear flow
  \item $V$ is shear force
  \item Q is first moment of area, units $m^3$ of some partial area of cross-section (dependent on question) about centroid
\end{itemize}
Shear flow:
\begin{align}
  q = \frac{VQ}{I} \quad [N/m].
\end{align}
Moment of inertia for for rectangular cross-section:
\begin{align}
  I = \frac{bh^3}{12}.
\end{align}
...
where
\begin{align}
  \bar{y} = (h/2 - y)\cdot 1/2 + y, 
\end{align}
thus,
\begin{align}
  Q(y) = \frac{b}{2}\left(\frac{h}{2}-y\right)^2 + by \left(\frac{h}{2}-2\right)\quad [m^3],
\end{align}
which is a quadratic in $y$. $\implies q(y)$ varies quadtraticlly in $y$.
\\\\
*Importantly, equal and opposite shears exist at a cross-section.
\\\\
\textbf{Summary: Bending of thin-walled beams}
\begin{itemize}
  \item Shear flow is shear force per unit distance $q = \frac{VQ}{I}$
  \item q varies along the length of a beam (due to a change in $V$) as well as
  across the depth of cross-section (due to a change in $\sigma(y) = \frac{-My}{I}$)
  \item The sum total of shear flow at any given cross-section is conserved
  \item Shear force in a segment of cross-section: 
  $F = \int q ds$
 \item Shear flow can exhibit complex patterns in its direction and
  magnitude as illustrated next
 \item Most significantly shear flow can cause unwanted torsion during
  bending
\end{itemize}

\textbf{Bending and Twisting}
\begin{itemize}
  \item Bending and Twisting: $\tau \neq \frac{VQ}{It}$
  \item Only Bending (No twisting) $\tau = \frac{VQ}{It}$. *Preferred for design.
  \item Only Twisting (No bending): $\tau \neq \frac{VQ}{It}$
\end{itemize}

\subsection{Shear Center}
